    ...
    <Paper sx={{
      ...
      <Box sx={{ p: 2.5, pb: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          ...
          <IconButton size="small" onClick={() => onUpdate(stock)} disabled={isLoading} sx={{ color: '#9ca3af', '&:hover': { color: '#1a1f36' }, flexShrink: 0 }}>
            <Edit fontSize="small" />
          </IconButton>
        </Box>

        <Box sx={{ mt: 2, display: 'flex', flexDirection: 'column', gap: 0.75, minWidth: 0 }}>
          {stock?.address && (
      ...
      <Box sx={{ mx: 2.5, p: 2, borderRadius: 2, background: status.bg, flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
      ...
      <Box sx={{ px: 2.5, pt: 2, pb: 2.5, mt: 'auto' }}>
...
function UnregisteredHospitalCard({ hospital, onRegister, isLoading }) {
  ...
  return (
    <Paper sx={{
      ...
      <Box sx={{ mx: 2.5, p: 2, borderRadius: 2, background: '#ebfffb', flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
        ...
        <Box sx={{ mb: 4, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
          <Box>
            ...
            <Typography sx={{ color: "#64748b", mt: 0.5, fontSize: '0.95rem' }}>
              Monitor and manage Anti-Snake Venom inventory across the facility network
            </Typography>
          </Box>
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            <Button
              variant="contained"
              startIcon={<Refresh />}
              onClick={() => { fetchStockData(); fetchSummary(); }}
              disabled={loading}
              sx={{
                bgcolor: "#1a1f36",
                color: "white",
            ...
            <>
              <Grid container spacing={2} sx={{ mb: 4 }}>
                {summaryCards.map((card) => (
                  <Grid item xs={6} sm={4} md={3} key={card.label} sx={{ display: 'flex' }}>
                    <Paper sx={{
                      p: { xs: 2, md: 2.5 },
                      borderRadius: 1,
                      display: 'flex',
              ...
              {registeredCount > 0 && (
                <>
                  ...
                  <Grid container spacing={2.5} sx={{ mb: 4, display: 'flex', alignItems: 'stretch' }}>
                    {stockData.filter(s => s.id).map(stock => (
                      <Grid item xs={12} sm={6} lg={4} xl={3} key={stock?.id || Math.random()} sx={{ display: 'flex' }}>
                ...
                <>
                  ...
                  <Grid container spacing={2.5} sx={{ display: 'flex', alignItems: 'stretch' }}>
                    {unregisteredHospitals.map(hospital => (
                      <Grid item xs={12} sm={6} lg={4} xl={3} key={hospital?.hospital_id || Math.random()} sx={{ display: 'flex' }}>
