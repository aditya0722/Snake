...
import {
  ...
  InputAdornment, Avatar, Stack, Chip,
} from '@mui/material';
import {
  Add, Search, LocationOn, PhotoCamera, Close, CalendarToday,
  ArrowForward, Edit, Delete, Person, AccessTime, CheckCircle, Pets,
  ...
  return (
    <Card sx={{
      ...
      <CardContent sx={{ flexGrow: 1, p: 2.5, pb: 1.5 }}>
        <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 0.75, mb: 1.5, minWidth: 0 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, minWidth: 0 }}>
            <Avatar sx={{ width: 20, height: 20, bgcolor: '#1a1f36', fontSize: '0.6rem', fontWeight: 700, flexShrink: 0 }}>
        ...
        <Box sx={{ mb: 4, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
          <Box>
            ...
            <Typography sx={{ color: "#64748b", mt: 0.5, fontSize: '0.95rem' }}>
              Monitor community reports and coordinates for active snake rescue efforts
            </Typography>
          </Box>
        ...
        <Box sx={{ maxWidth: 1400, mx: 'auto' }}>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            {summaryCards.map(card => (
              <Grid item xs={6} sm={3} key={card.label} sx={{ display: 'flex' }}>
                <Paper sx={{ p: { xs: 1.5, md: 2 }, borderRadius: 1, display: 'flex', alignItems: 'center', gap: 2, width: '100%', height: '100%' }}>
                  <Box sx={{ minWidth: 44, height: 44, borderRadius: 1, background: card.bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Typography sx={{ color: card.color, fontWeight: 800, fontSize: '1.2rem' }}>{card.value}</Typography>
                  </Box>
                  <Typography variant="caption" sx={{ color: '#9ca3af', fontWeight: 700, letterSpacing: '0.05em', lineHeight: 1.3, fontSize: '0.65rem' }}>{card.label}</Typography>
                </Paper>
              </Grid>
            ))}
          </Grid>
